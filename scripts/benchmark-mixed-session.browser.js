...
(() => {
  ...
  const options = { rounds: 32, sampleMs: 8000, primePressure: true, ...(globalThis.__n3onMixedSoakOptions ?? {}) };
  const report = globalThis.__n3onMixedSoak = {
  ...
  report.promise = (async () => {
    try {
      ...
      SaveSystem.updateTutorialProgress(p => { p.firstRunStage = 'completed'; });
      ...
      const session = { baseSeed: 550055, round: 54, objectiveMode: 'open', protocol: 'supreme-leo',
      ...
      globalThis.n3onRoundLifecycleSoak(options.rounds + 8);
      for (let i = 0; i < options.rounds; i++) {
        ...
        while (!game.scene.getScene('round-finished').sys.isActive()) {
          ...
          if (arena.bossEncounter && !bossDone) {
            ...
            snap('boss-active'); await wait(3000);
          ...
          if (arena.bossFlowPhase === 'loot-collection') {
            ...
            collectArena(arena);
            if (!arena.pickups.length && !arena.modPickups.length && !arena.bossLootLaunchesPending) arena.finishBossCollection();
          }
          await wait(100);
        }
        await wait(100);
        ...
        if (i + 1 < options.rounds) {
          ...
          // Repeat four equivalent Supreme encounters to measure history cost
          // independently of difficulty growth; use actual result/loading flow.
          startArenaLoad(finished, { reason: 'continue-next-round', session: { ...session,
            round: 54 + (i + 1) % 4, equippedMods: payload.equippedMods, modsEarned: payload.modsEarned }, message: 'Mixed lifecycle soak' });
