...
function fixture(config) {
  ...
  return { timeline, step, events, fired, poses, snaps };
}
for (const duration of [1, 1000, 1700, 2000, 4000]) test(`Echo releases at ${duration} ms with independent 12-second cooldown`, () => {
  const f = fixture(); f.step(0, true, true); f.step(duration, false, false, 90, 100);
...
test('cooldown rejects new recordings, including during replay', () => {
  const f = fixture(); f.step(0, true, true); f.step(1000, false);
  f.step(1, true, true); assert.equal(f.timeline.recording, false);
  f.step(1000, false); f.step(1, true, true); assert.equal(f.timeline.recording, false);
});
...
test('replay uses shortest rotation across wrap and preserves the recorded route', () => {
  ...
  f.step(1000, false, false, 100, 100, -Math.PI + .1);
...
test('shot-time mixed ammo, ordering, modifiers and grenade sequence survive later live changes', () => {
  ...
  live.damage = 999; live.texture = 'changed'; f.step(600, false);
...
test('reset cancels recording and replay without a snap or subsequent weapon event', () => {
  for (const duringReplay of [false, true]) {
    ...
    if (duringReplay) f.step(500, false);
