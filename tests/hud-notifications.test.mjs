...
test('notifications keep equal-priority order and promote critical queued notices', () => {
  ...
  q.update(3000); assert.equal(q.active.heading, 'critical');
  q.update(3000); assert.equal(q.active.heading, 'second');
  q.update(3000); assert.equal(q.active.heading, 'third');
  q.update(3000); assert.equal(q.active, null);
});
test('repeat suppression and queue capacity bound bursts without displacing higher priority notices', () => {
  const q = new HudNotificationQueue();
  for (let i = 0; i < 100; i++) q.submit(notice('repeat'));
  assert.equal(q.pending.length, 1);
  q.update(0); assert.equal(q.submit(notice('repeat')), false);
  q.update(3000); assert.equal(q.submit(notice('repeat')), false);
  q.update(5001); assert.equal(q.submit(notice('repeat')), true);
  q.clear();
  for (let i = 0; i < 12; i++) q.submit(notice('weekly'+i, 2));
  assert.equal(q.submit(notice('noise')), false);
  assert.equal(q.submit(notice('urgent', 4)), true);
  assert.equal(q.pending.length, 12); assert.equal(q.pending[0].heading, 'urgent');
});
test('invalid durations expire, encounter clear and event cancellation remove stale work', () => {
  const q = new HudNotificationQueue();
  q.submit({ ...notice('bad'), durationMs: NaN }); q.update(0); q.update(3000);
  ...
  q.cancelPrefix('redline:'); assert.equal(q.active, null); assert.equal(q.pending.length, 1);
