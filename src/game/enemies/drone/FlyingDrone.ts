...
export class FlyingDrone extends Enemy {
  ...
  constructor(scene: Phaser.Scene, x: number, y: number, stats: EnemyStats, sequence: number,
    variant: DroneVariant = 'standard', movement?: DroneMovement) {
    super(scene, x, y, 'enemy-drone', stats);
    this.variant = variant;
    this.flight = movement ?? new ArenaDroneStrafe(sequence);
    this.setDepth(10).setAlpha(.3);
    this.setVisualTintOverride(DRONE_VARIANTS[variant].color);
  ...
  updateFlight(now: number, dt: number, targetX: number, targetY: number, bounds: RectSpec,
    cadenceMs: number, fire: (angle: number) => void): void {
    const delta = Math.max(0, Math.min(.05, dt));
    this.elapsedMs += delta * 1000;
    if (now < this.disabledUntil) { this.setVelocity(0, 0); return; }
    const body = this.body as Phaser.Physics.Arcade.Body;
    const s = this.flightState;
    s.x = this.x; s.y = this.y; s.vx = body.velocity.x; s.vy = body.velocity.y;
    ...
    if (s.x !== this.x || s.y !== this.y) body.reset(s.x, s.y);
