...
export class RedlineVisualController {
  ...
  announce(text: string, now: number): void { this.notice.setText(text);this.noticeUntil=now+1600; }
  ...
  update(now: number, model: RedlineMomentum, remainingMs: number, terminalMs = -1, result?: RedlineResult): void {
    ...
    for(const rail of this.rails)rail.setAlpha(intensity);
