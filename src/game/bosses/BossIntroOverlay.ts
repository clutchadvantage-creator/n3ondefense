...
export class BossIntroOverlay {
  ...
  constructor(scene: Phaser.Scene, archetype: BossArchetype, onReady: () => void) {
    ...
    this.ready = new ArenaCommandButton(scene, 'READY // ENGAGE', () => {
      if (this.accepted) return false;
      this.accepted = true;
      onReady();
      return true;
    });
    this.root.add([this.veil, this.scanlines, this.frame, this.ghostA, this.ghostB, this.bossImage, heading, title, subtitle, directive]);
  ...
  resize(width: number, height: number): void {
    ...
    this.ready.setGamePosition(width * 0.5, height * 0.5 + 170 * scale, 280 * scale, 48 * scale);
