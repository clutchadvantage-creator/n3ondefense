...
const blockedAt=(x:number,y:number,walls:RectSpec[],padding=20):boolean=>walls.some((r)=>x>=r.x-padding&&x<=r.x+r.w+padding&&y>=r.y-padding&&y<=r.y+r.h+padding);
export const validateTopologyDraft=(draft:ArenaTopologyDraft,sites:PointSpec[]):{valid:boolean;failures:string[]}=>{
  const failures:string[]=[];
  const cell=40,cols=Math.ceil(draft.bounds.w/cell),rows=Math.ceil(draft.bounds.h/cell);
